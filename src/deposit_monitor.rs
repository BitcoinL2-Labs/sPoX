...
impl DepositMonitor {
    ...
    /// Parse a `Utxo` into a create deposit request for Emily
    ...
    pub fn get_pending_deposits(
        ...
        // TODO: cache this
        let monitored_addrs = self.monitored.keys().cloned().collect::<Vec<_>>();

        let utxos = self.context.bitcoin_client().get_utxos(&monitored_addrs)?;
